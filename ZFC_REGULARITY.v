...
Definition Rs (M:Ens) :=
  ~ (exists (n:nat) (L:list Ens) (A:Ens), (2 <= n) /\
       length L = n /\
       EQ A M /\
       Some A = nth_error L 0 /\
       Some A = nth_error L (pred n) /\ 
forall i, ((S i) < n) -> exists Xi Xsi, 
  (nth_error L i) = Some Xi 
       /\ (nth_error L (S i)) = Some Xsi  
    /\ IN Xsi Xi).



Lemma tail_length : forall A (L:list A) n, 1 <= n ->
  length L = n ->
  S (length (tl L)) = n.
Proof. destruct L; intros. 
  - subst. inversion H. 
  - assumption.
Qed. 

Lemma tail_nth : forall A (L:list A) xi i,
  nth_error L (S i) = Some xi ->
  nth_error (tl L) i = Some xi.
Proof. intros. destruct L. 
  - inversion H.
  - simpl in *. assumption. 
Qed. 

Theorem eps_ind_Rs : forall x : Ens,
        (forall y : Ens, IN y x -> Rs y) -> Rs x.
Proof. intros. induction x as [B e].
  intros [n [L [A H1]]]. 
  destruct H1 as [Hn1 [Hlength [HEQ [Hf [Hl HH]]]]].
  
  assert (S 0 < n) by omega.
  destruct (HH 0 H1) as [z [f [Hz [Hll HIN]]]].

  assert (IN f (sup B e))
    by (rewrite <- Hf in Hz; inversion Hz; subst z; 
    apply IN_sound_right with A; assumption).

  apply H in H2. unfold Rs in H2. apply H2. clear H2.

  exists n, ((tl L) ++ (cons f nil)), f.
  destruct n. inversion Hn1.

  assert (Htl: S (length (tl L)) = S n)
    by (apply tail_length; omega).
  ...
  - rewrite nth_error_app1; try omega.
    destruct L; try destruct L; simpl in Htl; try omega.
    symmetry. apply Hll.
  - inversion Htl.
  ...
  - inversion Htl.
    intros i HSi. 
    destruct (Nat.eqb_spec i (pred n)).
    + exists z, f.
      repeat split; try assumption.
      * rewrite nth_error_app1; try omega.
        try apply tail_nth. 
        rewrite e0, <- Hz, <- Hf.
        symmetry. simpl in Hl.
        replace (S (Init.Nat.pred n)) with n by omega.
        apply Hl. 
      * rewrite nth_error_app2; try omega.
        replace (S i - length (tl L)) with 0 by omega.
        reflexivity. 
    + assert (S (S i) < S n) by omega.
      destruct (HH (S i) H2) as [xi [xsi [E1 [E2 E3]]]].
      exists xi, xsi.
      repeat split;
      try rewrite nth_error_app1; try omega;
      try apply tail_nth; try assumption.
Qed. 

Theorem EQ_preserves_Rs :   
  forall a b, EQ a b -> (Rs a <-> Rs b).
Proof. intros. split; unfold Rs. 
  - intros H2 [n [L [A H1]]].
    destruct H1 as [Hn1 [Hlength [HEQ [Hf [Hl HH]]]]].    
    apply H2.
    exists n, L, A.
    repeat split; try assumption.
    eapply EQ_tran. apply HEQ. apply EQ_sym.
    assumption.
  - intros H2 [n [L [A H1]]].
    destruct H1 as [Hn1 [Hlength [HEQ [Hf [Hl HH]]]]].    
    apply H2.
    exists n, L, A.
    repeat split; try assumption.
    eapply EQ_tran. apply HEQ.  
    assumption.
Qed.

Check (eps_ind Rs) EQ_preserves_Rs eps_ind_Rs.


Import ListNotations.
Check (eps_ind Rs) EQ_preserves_Rs eps_ind_Rs.
Definition prf := (eps_ind Rs) EQ_preserves_Rs eps_ind_Rs.

Theorem my X : EQ X (Sing (Sing X)) -> False.
Proof.
intros xessx.
Print Rs.
assert (W: Rs X) by (exact (prf X)). (*pose (Q:= prf X).*)
unfold Rs in W.
apply W.
clear W.
exists 3.
...
{ exists X, (Sing X).
  split. auto.
  split. auto.
  apply IN_sound_right with (E':=(Sing (Sing X)));
  auto with zfc. }
induction i.
{ exists (Sing X), X.
  split. auto.
...
Definition Rs (M:Ens) :=
  ~ (exists (n:nat) (L:list Ens), (2 <= n) /\
       length L = n /\
       Some M = nth_error L 0 /\
       Some M = nth_error L (pred n) /\ 
forall i, ((S i) < n) -> exists Xi Xsi, 
  (nth_error L i) = Some Xi 
       /\ (nth_error L (S i)) = Some Xsi  
    /\ IN Xsi Xi).
...
Proof. intros. induction x.
  intros [n [L H1]].
  destruct H1 as [Hn1 [Hlength [Hf [Hl HH]]]].
  assert (S 0 < n) by omega.
  destruct (HH 0 H1) as [z [f [Hz [Hll HIN]]]].

  assert (IN f (sup A e))
    by (rewrite <- Hf in Hz; inversion Hz; subst z; assumption). 
  apply H in H2. unfold Rs in H2. apply H2. clear H2.

  exists n, ((tl L) ++ (cons f nil)).
  destruct n. inversion Hn1.
