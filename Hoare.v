...
Ltac find_rwinv :=
  match goal with
    H1: ?E = true,
    H2: ?E = false
    |- _ => rewrite H1 in H2; inversion H2
  end.
...
Proof. unfold hoare_triple.
  intro H. assert ({X --> 1} X = aeval {X --> 1} (X+1)).
  { apply H with {-->0}; constructor. reflexivity. }
  inversion H0.
Qed.
...
Proof.
  ...
  assumption. split; try assumption. reflexivity.
Qed.
...
Proof. eapply hoare_consequence_pre.
  apply hoare_asgn.
  intros st H. unfold assn_sub, t_update. assumption. Qed.
...
~ forall (a : aexp) (n : nat),
  ...
  {{fun st => st Y = n}}.
Proof. intro H.
  assert ((X ::= 3;; Y ::= X) / {--> 0} \\ {X --> 3 ; Y --> 3}) 
   by (eapply E_Seq; constructor; reflexivity).
  apply (H X 0 { --> 0} 
    ({ --> 0} & {X --> 3; Y --> 3})) in H0; try reflexivity. 
  inversion H0. 
Qed.
...
Proof. apply hoare_if;
  eapply hoare_consequence_pre; try  
  apply hoare_asgn;
  unfold bassn, assn_sub, t_update, assert_implies.
  - simpl. intros st [_ H].
...
Proof. unfold hoare_triple. intros.
  inversion H1; subst.
  - apply H with st; try split; 
    assumption. 
  - apply H0. split; try assumption.
    unfold bassn. intro H'.
    find_rwinv. 
Qed.
...
Proof. apply hoare_if.
  ...
  - intros st [H1 H2]. unfold bassn in H2.
    destruct (Nat.eqb_spec (st Y) 0).
    + omega.
    + apply Nat.eqb_neq in n.
      simpl in H2. rewrite n in H2. contradiction.
Qed.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_RepeatFalse : forall st st' st'' b1 c1,
      beval st' b1 = false ->
      ceval st c1 st' ->
      ceval st' (REPEAT c1 UNTIL b1 END) st'' ->
      ceval st (REPEAT c1 UNTIL b1 END) st''
...
Proof. apply E_RepeatTrue.
  reflexivity. eapply E_Seq; constructor; reflexivity.
Qed. 
...
Theorem hoare_repeat : forall P Q b c,
  {{fun st => P st }} c {{ fun st => Q st /\ (P st \/ bassn b st)}} ->
  {{P}} REPEAT c UNTIL b END {{fun st => Q st /\ bassn b st}}.
Proof. intros.
  intros st st' HR HP. remember (REPEAT c UNTIL b END) as re.
  induction HR; try inversion Heqre; subst.
  - split. unfold hoare_triple in H. apply H with st.
    assumption. assumption. assumption. 
  - unfold hoare_triple in H. 
    destruct (H _ _ HR1 HP) as [_ [HH | HH]].
    + apply IHHR2. assumption. assumption. 
    + unfold bassn in HH. try find_rwinv.
Qed.
...
Definition havoc_pre (X : string) (Q : Assertion) : Assertion :=
  (fun st => forall (n:nat), Q (st & {X --> n})).
  
Theorem hoare_havoc : forall (Q : Assertion) (X : string),
  {{ havoc_pre X Q }} HAVOC X {{ Q }}.
Proof. unfold hoare_triple, havoc_pre. intros. 
  inversion H; subst. apply H0. Qed.
