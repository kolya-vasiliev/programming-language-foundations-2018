...
Proof.
  ...
  - 
    inversion Ht; subst.
    destruct (IHt1 _ H2); destruct (IHt2 _ H4); right.
    + assert (exists x0 t0, t1 = tabs x0 T11 t0).
        eapply canonical_forms_fun; eauto.
      destruct H1 as [x0 [t0 HH]].    
      exists ([x0:=t2]t0). subst. auto.
    + destruct H0. eauto.
    + destruct H. eauto.
    + destruct H. eauto.
  - inversion Ht; subst.
...
Proof.
  ...
  - apply (preservation x0 y0) in Hhas_type; auto.
Qed.
...
Corollary unique_types : forall t Gamma T T',
  Gamma |- t \in T ->
  Gamma |- t \in T' ->
  T = T'.
Proof. induction t; intros;
  ...
  pose (IHt1 _ _ _ H4 H10).
  inversion e; auto. 
  
  pose (IHt _ _ _ H6 H12).
  inversion e; auto.

  pose (IHt2 _ _ _ H7 H15).
  inversion e; auto.
Qed.
