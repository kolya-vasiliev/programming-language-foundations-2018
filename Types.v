...
Example some_term_is_stuck :
  exists t, stuck t.
Proof. exists (tiszero ttrue). split;
...
Proof. intros t H H2.  
  inversion H2. clear H2. induction H0; try solve_by_inverts 2. 
  inversion H. inversion H1. inversion H1.
  apply IHstep. right. assumption.
Qed.

Lemma nvalue_is_nf : forall t t',
  nvalue t -> t ==> t' -> False. 
Proof. intros. assert (value t) by (right; assumption).
...
Ltac sbi n :=
  ...
  match type of T with Prop =>
    solve [ 
      inversion H; 
     try (exfalso; eapply nvalue_is_nf; eassumption);
      try reflexivity;
      match n with S (S (?n')) => subst; sbi (S n') end ]
  end end.

Theorem step_deterministic:
  deterministic step.
Proof with eauto. intros x y1 y2 H.
  generalize dependent y2. induction H; intros; try sbi 2;
  inversion H0; subst; try sbi 1;
   try (erewrite IHstep; try eassumption; try reflexivity).
Qed.
...
Proof with auto.
  ...
  - inversion IHHT; clear IHHT. 
    ...
    + right. inversion H as [t1']. exists (tsucc t1')...
  - inversion IHHT; clear IHHT; right. 
    + apply (nat_canonical t1 HT) in H. 
      destruct H. exists tzero... exists t... 
    + inversion H as [t1']. exists (tpred t1')...
  - inversion IHHT; clear IHHT. 
    + apply (nat_canonical t1 HT) in H. 
      destruct H; right. exists ttrue... exists tfalse...
    + inversion H as [t1']. right. exists (tiszero t1')...
Qed.
...
Ltac sb n x :=
  ...
  match type of T with Prop =>
    solve [ 
      inversion H; 
      try x;
      match n with S (S (?n')) => subst; (sb (S n') x) end ]
  end end.
...
Theorem preservation' : forall t t' T,
  ...
  t ==> t' ->
  |- t' \in T.
Proof. intros. generalize dependent T. 
  induction H0; intros; try sb 2 eauto.
Qed.
...
Theorem normalize_ex' : exists e',
  (P (C 3) (P (C 2) (C 1)))
  ==>* e'.
Proof. apply ex_intro with (C 6). normalize. Qed.
...
Lemma expan_fails : 
tif tfalse tzero ttrue ==> ttrue /\
|- ttrue \in TBool /\
~ (|- tif tfalse tzero ttrue \in TBool).
Proof. repeat try split; auto. apply has_type_not. Qed.
