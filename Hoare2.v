...
Proof. intros m.
  ...
  - apply hoare_consequence_pre with
    ...
    + apply hoare_while.
      ...
      * apply hoare_asgn.
      * intro st. unfold assn_sub, bassn, t_update. simpl.
        intros [H1 H2]. rewrite <- H1. apply parity_ge_2.
        destruct (st X). inversion H2. destruct n.
        inversion H2. omega.
...
(* power_sum (a) = 2^0 + 2^1 + ... + 2^a
  ...
  {{ Z = 2^X /\ Y = power_sum (X)}}
    WHILE !(X = m) DO
  {{ Z = 2^X /\ Y = power_sum (X) /\ X <> m}} ->>
{{ 2*Z = 2^(X + 1) /\ Y + 2*Z = power_sum (X+1) }}      
      Z ::= 2 * Z;;
{{ Z = 2^(X + 1) /\ Y + Z = power_sum (X+1) }}      
      Y ::= Y + Z;;
{{ Z = 2^(X + 1) /\ Y = power_sum (X+1) }}      
      X ::= X + 1
      {{ Z = 2^X /\ Y = power_sum (X) }}
    END
...
Proof. split.
  - eapply hoare_consequence_pre.
    apply hoare_asgn.
    intros st H. unfold assn_sub, t_update.
    simpl. omega.
  - intros P' H st H'. unfold hoare_triple in H.
    pose (H st (st & {X --> aeval st (Y+1)})).
    assert ((st & {X --> aeval st (Y + 1)}) X <= 5).
    { apply l. constructor. reflexivity. assumption. }
...
Theorem hoare_asgn_weakest : forall Q X a,
  is_wp (Q [X |-> a]) (X ::= a) Q.
Proof. unfold is_wp. split.
  - apply hoare_asgn.
  - unfold hoare_triple, assn_sub. intros P' H st H'.
    apply H with st. constructor. reflexivity. assumption.
Qed. 
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  {{ P }} HAVOC X {{ Q }} ->
  P ->> havoc_pre X Q.
Proof. unfold havoc_pre, hoare_triple, 
  assert_implies. intros. 
  apply H with st. constructor. assumption.
Qed.
...
Lemma l3' : forall m,
  ev m ->
  ~ev (S m).
Proof. induction m; intros H1 H2. inversion H2.
  apply IHm. inversion H2; subst; assumption. assumption. Qed.
...
Example factorial_dec (m:nat) : decorated :=
    {{ fun st => st X = m }} ->>
    {{ fun st => 1 = (real_fact m) / (real_fact (st X)) /\ st X <= m}}
  Y ::= 1
    {{ fun st => st Y = (real_fact m) / (real_fact (st X)) /\ st X <= m }};;
  WHILE !(X = 0)
  DO   {{ fun st => st Y = (real_fact m) / (real_fact (st X)) /\ (st X) <> 0 /\ st X <= m }} ->>
       {{ fun st => (st Y) * (st X) = (real_fact m) / (real_fact (st X - 1)) /\ st X <= m }}
     Y ::= Y * X
       {{ fun st => st Y = (real_fact m) / (real_fact (st X - 1)) /\ st X <= m}} ->>
       {{ fun st => st Y = (real_fact m) / (real_fact (st X - 1)) /\ st X - 1 <= m}};;
     X ::= X - 1
       {{ fun st => st Y = (real_fact m) / (real_fact (st X)) /\ st X <= m}}
  END
    {{ fun st => st Y = (real_fact m) / (real_fact (st X)) /\ st X <= m /\ st X = 0 }} ->>
    {{ fun st => st Y = (real_fact m) }}.

Lemma factorial_nonzero: forall n,
  real_fact n <> 0.
Proof. induction n; simpl; intro contra. 
  - inversion contra.
  - rewrite plus_comm in contra.
    rewrite mult_comm in contra.
    rewrite mult_n_Sm in contra. 
    apply Nat.eq_mul_0_l in contra. contradiction.
    intros contra2. inversion contra2.
Qed.

Lemma real_fact_divide : forall m n,
  n <= m ->
Nat.divide (real_fact n) (real_fact m).
Proof. induction m; intros.
- inversion H. apply Nat.divide_refl.
- destruct (Nat.eqb_spec n (S m)).
  + subst. apply Nat.divide_refl.
  + apply Nat.divide_mul_r. apply IHm. omega.
Qed.

Lemma real_fact_div: forall m n,
  (S n) <= m ->
  real_fact m / real_fact (S n) * (S n) =
  real_fact m / real_fact (n).
Proof with try apply factorial_nonzero. 
  induction m.
  intros. inversion H.
  intros. apply Peano.le_S_n in H.
  ...
  - rewrite e. simpl. 
    ...
    rewrite (mult_comm m).
    rewrite mult_n_Sm.
    rewrite (mult_comm (real_fact m)).
    rewrite Nat.divide_div_mul_exact;
      try apply Nat.divide_refl...
    rewrite Nat.div_same...
    apply mult_comm.
    rewrite (mult_comm m).
    rewrite mult_n_Sm.
    intros H'. apply Nat.eq_mul_0_l in H'.
    generalize H'... omega.
  - replace (real_fact (S m)) with 
      ((S m) * real_fact m) by reflexivity.
    rewrite Nat.divide_div_mul_exact;
    try apply real_fact_divide; try omega...
    rewrite Nat.divide_div_mul_exact;
    try apply real_fact_divide; try omega...
    rewrite <- mult_assoc.
    rewrite IHm; omega.
...
Proof. intro m. verify. 
  ...
  - destruct (st X); try omega.
    ...
    apply real_fact_div. assumption.
  - replace (real_fact 0) with 1.
    apply Nat.div_1_r. reflexivity.
Qed.
...
Proof. intros. destruct n. inversion H.
