...
(** The simply typed lambda-calculus (STLC) is a tiny core
    ...
    language in s	ome form (functions, procedures, methods, etc.).
...
Inductive substi (s:tm) (x:string) : tm -> tm -> Prop :=
  ...
  | s_tabs1 : forall x' T t1,
      x = x' ->
      substi s x (tabs x' T t1) (tabs x' T t1)
  | s_tabs2 : forall x' T t1 t2,
      x <> x' ->
      substi s x t1 t2 -> 
      substi s x (tabs x' T t1) (tabs x' T t2)
...
Lemma step_example5 :
  ...
  ==>* idB.
Proof. eapply multi_step. apply ST_App1.
  apply ST_AppAbs. auto.
...
Proof. intro. inversion H.
  ...
  inversion H1. subst.
  inversion H7. subst.
  inversion H5. subst. rewrite update_eq in H4.
  inversion H8. subst. rewrite update_eq in H6.
  rewrite H4 in H6. inversion H6. 
  
  clear H0 H1 H5 H7 H8 H4 H6 H.   

  generalize dependent T12. 
  induction T11; intros;  
  inversion H3; eauto.
Qed.  
