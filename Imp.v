...
Proof. induction b as [| |a1 a2|a1 a2|b1 IH1|b1 IH1 b2 IH2];
  simpl; 
...
Definition max_optimus (a:aexp) : aexp := ANum (aeval a).

Theorem max_optimus_correct : forall a, aeval a = aeval (max_optimus a).
Proof. simpl. reflexivity. Qed.

Fixpoint optimize_const (b : bexp) : bexp :=
  match b with
  | BTrue       => BTrue
  | BFalse      => BFalse
  | BEq a1 a2   => BEq a1 a2
  | BLe a1 a2   => BLe a1 a2
  | BNot b1     => match optimize_const b1 with
                   | BTrue => BFalse
                   | BFalse => BTrue
                   | b1' => BNot b1'
                   end
  | BAnd b1 b2 => 
      match ((optimize_const b1), (optimize_const b2)) with
      | (BTrue, x) => x
      | (BFalse, _) => BFalse
      | (x, BTrue) => x
      | (_, BFalse) => BFalse
...
Theorem optimize_const_sound : forall b,
  beval (optimize_const b) = beval b.
Proof. induction b; try reflexivity.
  - destruct (optimize_const b) eqn:Heq;
     simpl; rewrite <- IHb;
     rewrite Heq; reflexivity.
  
  - destruct (optimize_const b1) eqn:Heq1;
    destruct (optimize_const b2) eqn:Heq2;
    simpl; try rewrite Heq1; try rewrite Heq2; 
         try rewrite <- IHb1; try rewrite <- IHb2;
         try (symmetry; try apply andb_true_r; 
            try apply andb_false_r);
    try reflexivity. 
Qed.

Fixpoint optimize_aexp_max (b : bexp) : bexp :=
  match b with
  | BTrue       => BTrue
  | BFalse      => BFalse
  | BEq a1 a2   => BEq (max_optimus a1) (max_optimus a2)
  | BLe a1 a2   => BLe (max_optimus a1) (max_optimus a2)
  | BNot b1     => BNot (optimize_aexp_max b1)
  | BAnd b1 b2 => BAnd (optimize_aexp_max b1) (optimize_aexp_max b2)
  end.

Theorem optimize_aexp_max_sound : forall b, 
  beval (optimize_aexp_max b) = beval b.
Proof. intros b. induction b;
  simpl; try rewrite IHb; try rewrite IHb1; try rewrite IHb2;
  reflexivity. Qed.

Definition final_optimize (b:bexp) : bexp := 
  optimize_const (optimize_aexp_max b).

Theorem final_optimize_sound : forall b, 
  beval (final_optimize b) = beval b.
Proof. intros b. unfold final_optimize. 
  rewrite optimize_const_sound, optimize_aexp_max_sound. 
  reflexivity. Qed.
...
| E_Beq :  forall e1 n1 e2 n2, e1 \\ n1 -> e2 \\ n2 -> 
  bevalR (BEq e1 e2) (beq_nat n1 n2) 
| E_Ble : forall e1 n1 e2 n2, e1 \\ n1 -> e2 \\ n2 -> 
  bevalR (BLe e1 e2) (leb n1 n2) 
| E_Not : forall bex b, bevalR bex b -> bevalR (BNot bex) (negb b) 
| E_BAnd : forall bex1 b1 bex2 b2, bevalR bex1 b1 -> bevalR bex2 b2 ->
  bevalR (BAnd bex1 bex2) (b1 && b2). 
...
Proof. 
  ...
  - generalize dependent bv. 
    induction b;
      intros; simpl; simpl in H; subst; constructor;
...
Proof. apply E_Seq with { X --> 2 ; Y --> 0 }.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nwR_Ass  : forall a1 x,
      no_whilesR (x ::= a1)
  | nwR_Seq : forall c1 c2,
      no_whilesR c1 ->
      no_whilesR c2 ->
      no_whilesR (c1 ;; c2)
  | nwR_If : forall b c1 c2,
      no_whilesR c1 ->
      no_whilesR c2 ->
...
Proof. split; intros.
  - induction c; 
    ...
    inversion H.
  - induction H; simpl;  
      try rewrite IHno_whilesR1, IHno_whilesR2;
    reflexivity. Qed.
...
Theorem no_whiles_terminating : forall c st,
  no_whilesR c -> 
  exists st', c / st \\ st'.
Proof. intros. generalize dependent st. induction H; intros.
  - exists st. constructor.
  - exists (st & {x --> aeval st a1}). constructor. reflexivity.
  - destruct (IHno_whilesR1 st) as [st' E].
    destruct (IHno_whilesR2 st') as [st'' E2].
    exists st''. apply E_Seq with st'; assumption.
  - destruct (IHno_whilesR1 st) as [st' E].
    destruct (IHno_whilesR2 st) as [st'' E2].
    destruct (beval st b) eqn:He; 
    [exists st'; apply E_IfTrue | exists st''; apply E_IfFalse];
         assumption.
Qed.
...
| h :: t => match h with
            ...
            | SPlus => match stack with
                       ...
                       | n :: [] => s_execute st stack t  
                       | n1 :: n2 :: q => s_execute st ((n2 + n1) :: q) t
                       end 
...
Lemma s_execute_concat : forall st cl1 cl2 stack,
  s_execute st stack (cl1 ++ cl2) = s_execute st (s_execute st stack cl1) cl2.
Proof. intros st cl1 cl2. induction cl1; intros stack.
  - reflexivity. 
  - destruct a; destruct stack; 
      try apply IHcl1; 
      try destruct stack; apply IHcl1.
Qed.

Theorem s_compile_correct : forall (st : state) (e : aexp) stack,
  s_execute st stack (s_compile e) = aeval st e :: stack.
Proof. 
  induction e; intros; try reflexivity; simpl; 
    rewrite s_execute_concat, s_execute_concat;
...
Theorem beval_short_correct : forall st b, 
  beval_short st b = beval st b.
Proof. reflexivity. Qed.
...
Theorem while_stops_on_break : forall b c st st',
  ...
  c / st \\ SBreak / st' ->
  (WHILE b DO c END) / st \\ SContinue / st'.
Proof. intros. apply E_WhileBreak; assumption. Qed. 
...
Proof. intros. remember (WHILE b DO c END) as C. 
...
Proof. intros. 
  ...
  - inversion H1; subst.
    ...
    destruct (IHceval1 _ _ H7). inversion H3.
  - inversion H0; subst. 
    destruct (IHceval _ _ H3). inversion H2.
    destruct (IHceval _ _ H6). split; assumption.
  - inversion H1; subst.
  ...
  - inversion H2; subst. 
      rewrite H in H8. inversion H8.
      destruct (IHceval1 _ _ H6); subst.
      destruct (IHceval2 _ _ H10); subst. split; reflexivity.
      destruct (IHceval1 _ _ H9). inversion H4.
  - inversion H1; subst. 
      rewrite H in H7. inversion H7.
      destruct (IHceval _ _ H5). inversion H3.
      destruct (IHceval _ _ H8); subst.
      split; reflexivity.
Qed. 
...
(** As for expressions, we can use a few [Notation] declarations to make 
    reading and writing Imp programs more convenient. *)
Bind Scope comf_scope with com.
Notation "'SKIP'" :=
   CSkip : comf_scope.
Notation "x '::=' a" :=
  (CAss x a) (at level 60) : comf_scope.
Notation "c1 ;; c2" :=
  (CSeq c1 c2) (at level 80, right associativity) : comf_scope.
Notation "'WHILE' b 'DO' c 'END'" :=
  (CWhile b c) (at level 80, right associativity) : comf_scope.
Notation "'IFB' c1 'THEN' c2 'ELSE' c3 'FI'" :=
  (CIf c1 c2 c3) (at level 80, right associativity) : comf_scope.
Notation "'FOR' '(' c1 ',' c2 ',' c3 ')' 'DO' c4 'END'" :=
  (CFor c1 c2 c3 c4) (at level 80, right associativity) : comf_scope.

(** The following declaration is needed to be able to use the
    notations in match patterns. *)
Open Scope comf_scope.
...
Definition fact_in_coq : com :=
  Z ::= X;;
  Y ::= 1;;
  FOR ( (Y ::= 1) , (Z = 0) , (Z ::= Z - 1) ) DO (Z ::= Z - 1)  END ;;
  WHILE ! (Z = 0) DO
    Y ::= Y * Z;;
    Z ::= Z - 1 
  END.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      (WHILE b DO c END) / st' // st'' ->
      (WHILE b DO c END) / st // st''
  | E_For : forall st st' st'' b c c1 c2,
      c1 / st // st' ->
      (WHILE b DO c ;; c2 END) / st' // st'' ->
      (FOR ( c1 , b , c2 ) DO c END) / st // st''
...
Proof.
  ...
  - assert (st' = st'0).
    ...
    apply IHE1_2. rewrite H. assumption.
 Qed.
